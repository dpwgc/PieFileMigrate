...
func dfsFileTree(node *fileTreeNodeModel) {
	...
	if !node.IsDir {
		...
		if !storageHandler.CheckFile(node.Path) && node.ModTime.Unix() > (time.Now().Unix()-base.Config.Server.MigrateFileTimeLimit) {
			// 异步迁移文件至其他服务器
